...
@registry.command("verify-dns")
@click.argument("domain", default="aamregistry.io")
@click.pass_context
def registry_verify_dns(ctx: click.Context, domain: str) -> None:
    """Verify DNS nameserver configuration for a registry domain.

    Checks that the domain's NS records point to the expected
    Google Cloud DNS nameservers.

    Examples::

        aam registry verify-dns
        aam registry verify-dns aamregistry.io
        aam registry verify-dns dev.aamregistry.io
    """
    console: Console = ctx.obj["console"]

    from aam_cli.utils.dns import verify_dns

    console.print(f"Verifying DNS for [bold]{domain}[/bold]...\n")
    ...
    # -----
    # Display resolved nameservers
    # -----
    table = Table(title="Nameservers", show_header=True, header_style="bold")
    table.add_column("Nameserver", style="cyan")
    table.add_column("Status")

    expected_lower = {ns.lower() for ns in result.expected_nameservers}

    for ns in result.resolved_nameservers:
        if ns.lower() in expected_lower:
            table.add_row(ns, "[green]expected[/green]")
        else:
            table.add_row(ns, "[yellow]unexpected[/yellow]")

    for ns in result.missing_nameservers:
        table.add_row(ns, "[red]missing[/red]")
    ...
    # -----
    if result.is_valid:
        console.print(f"[green]✓[/green] DNS is correctly configured for {domain}")
    else:
        console.print(f"[red]✗[/red] DNS is NOT correctly configured for {domain}")
        if result.missing_nameservers:
            console.print(f"  Missing: {', '.join(result.missing_nameservers)}")
        console.print(
            "\n  Update nameservers at your domain registrar and wait for propagation (up to 48h)."
        )
        ctx.exit(1)
